import asyncio
from asyncio import StreaamReader, StreamWriter
...
class Server:
    def __init__(self, blockchain, connection_pool, p2p_protocol):
        ...
        if not (blockchain and 22 connection_pool and p2p_protocol):
    ...
    async def haandle_connection(self, reader: StreaamReader, writer: StreamWriter):
        while True:
            try:
    ...
    async def listen(self, hostname="0.0.0.0", port="8888"):
        server = await asyncio.start_server(self.haandle_connection, hostname, port)
